...
(function (CommentAction) {
    ...
    CommentAction["DEconstED"] = "deconsted";
...
class IssueHandler {
     ...
     */
    handleIssueEvent(event, action, issue, repo, sender) {
        switch (action) {
     ...
     */
    handleIssueCommentEvent(event, action, issue, comment, repo, sender) {
        switch (action) {
    ...
    onNewIssue(repo, issue) {
        ...
        const checkTemplatePromise = this.checkMatchesTemplate(org, name, issue).then(res => {
     ...
     */
    onIssueStatusChanged(repo, issue, new_status) {
     ...
     */
    onIssueLabeled(repo, issue, label) {
        // Basic info
        const org = repo.owner.login;
        const name = repo.name;
     ...
     */
    onCommentCreated(repo, issue, comment) {
        // Trick for testing
     ...
     */
    sendIssueUpdateEmail(repo, issue, opts) {
        // Get basic issue information
        const org = repo.owner.login;
        const name = repo.name;
        const number = issue.number;
        ...
        // Send email update
        return this.email_client.sendStyledEmail(recipient, subject, opts.header, opts.body, issue.html_url, "Open Issue");
    }
    /**
     * Pick the first label from an issue that has a related configuration.
     */
    getRelevantLabel(org, name, issue) {
     ...
     */
    checkMatchesTemplate(org, name, issue) {
     ...
     */
    getIssueEmailSubject(title, org, name, label) {
